<template>
  ...
  <el-dialog v-model="isWC" title="人机验证" width="40%" :before-close="handleClose">
    <div class="verification-flex">
      <span>扫码下方二维码，关注后回复：<strong><span style="color:blue;">link</span></strong>，获取短链跟踪系统人机验证码</span>
...
const login = (formEl) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      ...
      if (domain === 'shortlink.magestack.cn' || domain === 'shortlink.xiuzi.com') {
        isWC.value = true
        return
      }
      const res1 = await API.user.login(loginForm)
